...
do_mknod(int argc, const char ** argv)
{
	...
	int		minor;
        ...
        if(argc < 5) {
            fprintf(stderr, "Not enough arguements for mknod");
...
#if	HAVE_LINUX_MOUNT
        if(argc < 2) {
            fprintf(stderr, "Not enough arguments for umount");
...
do_cmp(int argc, const char ** argv)
{
	...
	struct	stat	statBuf2;
        ...
        if(argc < 3) {
            fprintf(stderr, "Not enough arguments for cmp");
...
do_setenv(int argc, const char ** argv)
{
	...
	char *		str;
        ...
        if(argc < 3) {
            fprintf(stderr, "Not enough argument for setenv");
...
do_printenv(int argc, const char ** argv)
{
	...
	while (*env)
	{
		if ((strlen(*env) > (unsigned int)len) && (env[0][len] == '=') &&
...
do_where(int argc, const char ** argv)
{
	...
	BOOL		found;
        ...
        if(argc < 2) {
            fprintf(stderr, "Not enough arguments for where");
