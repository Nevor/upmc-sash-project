...
/* Added by Roven Gabriel
 * This function expands an environment variable into a filetable
 */
void
expandVar(const char * varName, const char *** retFileTable)
{
    char ** result;
    int size;
    ...
    if(strcmp(varName, "$$") == 0) {
    ...
    } else if(size > 2 && varName[1] == '[' && varName[size - 1] == ']') {
        *result = (char *)malloc(sizeof(char*) * 10);

        struct ast_st* ast = parseStr(varName);

        if(ast != NULL) {
            int value = evalArith(ast);
        ...
        *result = getenv(varName + 1);
    }

    if (*result == NULL) 
        *result = "";

    *retFileTable = (const char**)result;
}
	...
	*retArgv = NULL;
	...
	while (*cp)
	{
		...
		if (quotedWildCards && unquotedWildCards)
		{
			...
			return FALSE;
		} else if (quotedVar && unquotedVar) {
			fprintf(stderr,
				"Cannot use quoted and unquoted vars\n");
		 ...
		 */
                if (unquotedVar) {
                        expandVar(argument, &fileTable);
                        fileCount = 1;

                } 
                else if (unquotedWildCards)
		{
			/*
